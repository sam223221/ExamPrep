...
def is_terminal(state: Piles) -> bool:
    ...
    """
    ## TODO ##
    # A pile of 1 or 2 cannot be split into two *unequal* parts.
    # return all(____ for pile in state)
    ##########
...
def utility_of(state: Piles) -> int:
    ...
    """
    ## TODO ##
    # The game starts with 1 pile and the USER goes first.
    # Each split adds 1 pile, so len(state) tracks whose turn it is.
    #   len=1 (odd)  → user's turn    (start)
    #   len=2 (even) → computer's turn (after user's 1st move)
    # At a terminal state the player whose turn it is has LOST (can't move).
    #
    # if not is_terminal(state):
    #     return ____
    # if len(state) % 2 == ____:    # even → computer's turn → computer lost
    #     return ____
    # else:                          # odd  → user's turn → user lost
    #     return ____
    ##########
    raise NotImplementedError("Implement this function")
...
def successors_of(state: Piles) -> list[Piles]:
    ...
    #     if pile <= ____:        # skip piles that can't be split
    #         continue
    #     for split in split_pile_options(____):
    #         new_state = state[:i] + ____ + state[i + 1:]
...
def split_pile_options(pile: int) -> list[Piles]:
    ...
    #     k = ____ - j
    #     if j < k:               # only keep j < k to avoid duplicates like [6,1] and [1,6]
    #         options.append([j, k])
    # return options
    ##########
    raise NotImplementedError("Implement this function")
